...
export function ReviewsCarousel() {
  ...
  return (
    <section className="mx-auto w-full max-w-7xl px-4 pb-12 pt-4 sm:pb-14 sm:pt-6 lg:pb-16">
      <div className="relative overflow-hidden rounded-[28px] border border-slate-200/80 bg-[linear-gradient(180deg,#ffffff_0%,#f8fbff_100%)] px-4 py-8 shadow-[0_24px_60px_rgba(15,23,42,0.10)] sm:px-6 sm:py-10 lg:px-8">
        ...
        <div className="text-center">
          ...
          <h3 className="mt-4 text-2xl font-semibold tracking-tight text-slate-950 sm:text-3xl lg:text-4xl">
          ...
          <div className="mt-5 flex flex-wrap items-center justify-center gap-2 text-xs text-slate-600 sm:gap-3 sm:text-sm">
            <span className="rounded-full border border-slate-200 bg-white px-3 py-1">4.8/5 average rating</span>
            <span className="rounded-full border border-slate-200 bg-white px-3 py-1">9,000+ active businesses</span>
            <span className="rounded-full border border-slate-200 bg-white px-3 py-1">Trusted in 120+ cities</span>
          </div>
        </div>
          ...
          <div className="reviews-track flex w-max gap-4 [animation:reviewsMarquee_32s_linear_infinite] sm:gap-5">
            {scrollingReviews.map((review, index) => (
              <article
                ...
                className="group w-[290px] shrink-0 rounded-2xl border border-slate-200/90 bg-white p-4 shadow-[0_14px_28px_rgba(15,23,42,0.07)] transition-transform duration-300 hover:-translate-y-1 sm:w-[360px] sm:p-5"
              >
                ...
                <div className="min-h-[120px] text-sm leading-7 text-slate-700">&ldquo;{review.quote}&rdquo;</div>
                <div className="mt-4 border-t border-slate-100 pt-3">
                  <div className="text-base font-semibold text-slate-900">{review.name}</div>
                  <div className="text-xs text-slate-500">
                    {review.role} · {review.business}
                  </div>
                  <div className="mt-1 inline-flex rounded-full bg-indigo-50 px-2.5 py-1 text-[11px] font-medium text-indigo-700">
                    {review.city}
                  </div>
                </div>
              </article>
